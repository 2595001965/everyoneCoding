...
export function ConflictEditor({ onApplied }: ConflictEditorProps): JSX.Element {
  ...
  const [aiRequest, setAiRequest] = useState<{ path: string; instruction: string; context: string } | null>(null);
  ...
  const choose = useCallback((path: string, blockIndex: number, resolution: ConflictResolution) => {
    setChoices((prev) => ({ ...prev, [path]: { ...(prev[path] ?? {}), [blockIndex]: resolution } }));
  }, []);
  ...
  const apply = useCallback(async () => {
    ...
    for (const file of effective) {
      ...
      for (const block of file.blocks) {
        ...
        if (resolution !== undefined && resolution !== 'unresolved') fileChoices[block.index] = resolution;
      }
      const resolved = resolveConflictFile(file, fileChoices);
      const result = await api.applyResolution({ path: file.path, content: resolved.content, message });
      if (!result.ok) {
  ...
  if (files.length === 0) {
    return <EmptyState title="当前没有冲突" description="需要先执行合并或拉取并产生冲突，这里才会出现待解决内容。" />;
  }

  return (
    <div className="ec-conflict-editor" data-testid="conflict-editor" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="ec-conflict-editor__summary" role="status" data-testid="conflict-summary">
        共 {summary.files} 个文件、{summary.blocks} 个冲突块，未解决 <strong>{summary.unresolved}</strong> 块
      </div>

      {files.map((file) => (
        <section key={file.path} className="ec-conflict-editor__file" data-testid={`conflict-file-${file.path}`}>
          <header style={{ fontWeight: 600, fontFamily: 'monospace' }}>{file.path}</header>

          <div
            className="ec-conflict-editor__cols"
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, fontWeight: 600, color: 'var(--ec-color-text-secondary)' }}
          >
          ...
          {file.blocks.map((block) => {
            ...
            const resolvedLines = resolution === 'unresolved' ? [] : resolveBlock(block, resolution);
            return (
              <div key={block.index} className="ec-conflict-editor__block" data-testid={`conflict-block-${file.path}-${block.index}`}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 4 }}>
                  ...
                  <Tag color={resolution === 'unresolved' ? 'warning' : 'success'} data-testid={`resolution-${file.path}-${block.index}`}>
      ...
      {aiRequest !== null && (
        <div className="ec-conflict-editor__ai" role="status" data-testid="ai-merge-request">
          <div>
            已将 {aiRequest.path} 的冲突交给 AI 合并（这里只展示将要发送的指令与上下文，不接受手写代码）：
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <label style={{ color: 'var(--ec-color-text-secondary)' }}>
          ...
          <input
            ...
            style={{ marginLeft: 8, padding: '4px 8px', border: '1px solid var(--ec-color-border)', borderRadius: 6, background: 'var(--ec-color-surface)', color: 'var(--ec-color-text)' }}
          />
        </label>
        <Button size="sm" variant="primary" onClick={apply} disabled={!allResolved} loading={busy} data-testid="conflict-apply">
          应用解决结果
        </Button>
        ...
        {notice !== null && (
          <span role="status" style={{ color: 'var(--ec-color-text-secondary)' }} data-testid="conflict-notice">
            {notice}
          </span>
        )}
      </div>
...
function CodeColumn({ lines, testId, placeholder }: { lines: readonly string[]; testId: string; placeholder?: string }): JSX.Element {
  return (
    <pre
      data-testid={testId}
      style={{
