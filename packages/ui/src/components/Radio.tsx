    ...
    <div className={cx('ec-radio-group', className)} role="radiogroup">
      {React.Children.map(children, (child) => {
        ...
        return React.cloneElement(child as React.ReactElement<RadioProps>, {
          ...
          onChange: () => setVal(String((child as React.ReactElement<RadioProps>).props.value ?? '')),
...
export const Radio = React.forwardRef<HTMLInputElement, RadioProps>(function Radio(props, ref) {
  const { checked, defaultChecked = false, onChange, label, className, disabled, id, value, ...rest } = props;
  ...
  return (
    <label className={cx('ec-radio', disabled && 'ec-radio--disabled', className)} htmlFor={controlId}>
      <input
        ref={ref}
        id={controlId}
        type="radio"
