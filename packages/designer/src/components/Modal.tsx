...
export function ModalRenderer({ node, mode, scope, children }: ComponentRenderProps): JSX.Element {
  ...
  if (mode === 'design') {
    ...
    return (
      <div className="ecd-modal ecd-modal--design" style={style} role="dialog" aria-label={title || '弹窗'} data-component="Modal" data-mode={mode}>
        <div className="ecd-modal__header">{title || '弹窗标题'}</div>
        <div className="ecd-modal__body">{children ?? <span className="ecd-placeholder">弹窗内容</span>}</div>
  ...
  if (footerType !== 'none') modalProps.footer = <span className="ecd-modal__footer-ph">操作区</span>;

  return (
    <span data-component="Modal" data-mode={mode} data-mask-closable={maskClosable} data-footer-type={footerType}>
...
export const ModalMeta: ComponentMeta = {
  ...
  propSchema: {
    fields: [
      ...
      { key: 'footerType', label: '底部类型', type: 'enum', group: '内容', default: 'default', options: [
