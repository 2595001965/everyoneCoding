...
export function StashPanel({ onChanged }: StashPanelProps): JSX.Element {
  ...
  const applyEntry = useCallback(
    async (entry: GitStashEntry, drop: boolean) => {
      const result = drop ? await api.stashApply(entry.index, true) : await api.stashApply(entry.index, false);
      if (result.ok) {
        setNotice(drop ? `已恢复并删除 stash@{${entry.index}}` : `已恢复 stash@{${entry.index}}（保留记录）`);
        await reload();
        onChanged?.();
      } else {
  ...
  return (
    <div className="ec-stash-panel" data-testid="stash-panel" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div className="ec-stash-panel__new" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <Input
          aria-label="暂存说明"
          placeholder="暂存说明（可选）"
          value={message}
        ...
        {notice !== null && (
          <span role="status" style={{ color: 'var(--ec-color-text-secondary)' }} data-testid="stash-notice">
            {notice}
          </span>
        )}
      </div>
      ...
      {!loading && entries.length === 0 && (
        <EmptyState title="暂无暂存记录" description="「暂存当前改动」可以临时收起未完成的工作，之后再恢复。" />
      )}
      ...
      {!loading && entries.length > 0 && (
        <ul className="ec-stash-panel__list" data-testid="stash-list">
          {entries.map((entry) => (
            ...
            >
              <Tag color="neutral">stash@{'{'}{entry.index}{'}'}</Tag>
              ...
              <span style={{ color: 'var(--ec-color-text-secondary)' }} data-testid={`stash-time-${entry.index}`}>
                {formatTime(entry.createdAt)}
              </span>
              <button type="button" onClick={() => void applyEntry(entry, true)} data-testid={`stash-pop-${entry.index}`} style={linkBtn}>
                恢复并删除
              </button>
              <button type="button" onClick={() => void applyEntry(entry, false)} data-testid={`stash-apply-${entry.index}`} style={linkBtn}>
                仅恢复
              </button>
              <button
                type="button"
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button size="sm" variant="danger" onClick={confirmDrop} data-testid="stash-drop-confirm">
              删除
            </Button>
          </>
        }
      >
        <p>
          即将删除 <code>stash@{'{'}{pendingDrop?.index ?? 0}{'}'}</code>
          {pendingDrop !== null && pendingDrop.message.length > 0 ? `（${pendingDrop.message}）` : ''}。删除后这些改动不可恢复，确认继续？
        </p>
      </Modal>
    </div>
  );
