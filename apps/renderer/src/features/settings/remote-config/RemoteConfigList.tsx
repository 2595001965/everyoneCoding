...
const STATUS_LABEL: Record<string, { text: string; color: 'neutral' | 'success' | 'danger' | 'warning' }> = {
    ...
    <section className="ec-ai__section" aria-label="远程配置源">
      <header className="ec-ai__section-head">
        <div>
          ...
          <p className="ec-ai__hint">
            直连你填写的 URL 拉取配置（不经平台服务端）；未配置任何源时该能力关闭，拉取失败会自动回退本地缓存。
          </p>
        </div>
      ...
      {sources.length === 0 ? (
        ...
        <ul className="ec-ai__sources">
          {sources.map((source) => {
            const status = STATUS_LABEL[source.lastStatus ?? 'idle'] ?? STATUS_LABEL['idle' as const];
            return (
              <li
                key={source.id}
                className={source.id === selectedId ? 'ec-ai__source ec-ai__source--active' : 'ec-ai__source'}
              >
                <button type="button" className="ec-ai__source-main" onClick={() => onSelect(source)}>
                  <span className="ec-ai__name">
                    {source.name}
                    <Tag color={source.enabled ? 'success' : 'neutral'}>{source.enabled ? '启用' : '停用'}</Tag>
                    <Tag color={status?.color ?? 'neutral'}>{status?.text ?? '未知'}</Tag>
                  </span>
                  <code className="ec-ai__url">{source.url}</code>
                  <span className="ec-ai__hint">
                    {source.lastFetchAt ? `上次拉取：${new Date(source.lastFetchAt).toLocaleString()}` : '尚未拉取'}
                    {source.lastError ? ` · ${source.lastError}` : ''}
                    {source.publicKey ? ' · 已配置公钥' : ' · 未校验签名'}
                  </span>
                </button>
                <span className="ec-ai__row-actions">
                  <Button size="sm" variant="secondary" onClick={() => onFetch(source.id)} disabled={busy}>
                    立即拉取
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => onToggle(source.id, !source.enabled)} disabled={busy}>
                    {source.enabled ? '停用' : '启用'}
                  </Button>
                  <Button size="sm" variant="ghost" onClick={() => onRemove(source)} disabled={busy}>
                    删除
                  </Button>
                </span>
              </li>
