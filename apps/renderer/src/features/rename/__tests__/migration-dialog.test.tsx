...
async function renderDialog(
  ...
  api: FakeRenameApi;
  target: Awaited<ReturnType<FakeRenameApi['listTargets']>>[number];
}> {
...
describe('MigrationDialog', () => {
  it('生成 SQL 预览两栏、影响行数与默认不执行文案', async () => {
    ...
    expect(
      within(forward).getByText(/ALTER TABLE users RENAME COLUMN login_button TO login_submit/),
    ).toBeInTheDocument();
    expect(
      within(rollback).getByText(/ALTER TABLE users RENAME COLUMN login_submit TO login_button/),
    ).toBeInTheDocument();
  ...
  it('高危操作提示（requiresSecondConfirm 为真时两段确认）', async () => {
    ...
    api.planMigration = async (input) => {
      const preview = (await original(input)) as Awaited<
        ReturnType<FakeRenameApi['planMigration']>
      >;
