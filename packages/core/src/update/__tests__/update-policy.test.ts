...
import { compareVersions, isNewerVersion, isValidVersion, parseVersion } from '../update-types';
import {
...
describe('自动检查节奏（NFR-U-02：启动不被网络阻塞）', () => {
  ...
  it('首次启动检查一次', () => {
    expect(
      decideCheck({ settings: settings(), lastCheckAt: null, now, online: true }),
    ).toMatchObject({
      shouldCheck: true,
      reason: 'first-run',
    });
  ...
  it('间隔到期后再次检查', () => {
    ...
    expect(decideCheck({ settings: settings(), lastCheckAt, now, online: true }).reason).toBe(
      'interval-due',
    );
  });
  ...
  it('关闭自动检查 / 离线时都不检查，且离线不是错误', () => {
    expect(
      decideCheck({
        settings: settings({ autoCheck: false }),
        lastCheckAt: null,
        now,
        online: true,
      }),
    ...
    expect(
      decideCheck({ settings: settings(), lastCheckAt: null, now, online: false }),
    ).toMatchObject({
      shouldCheck: false,
      reason: 'offline',
    });
