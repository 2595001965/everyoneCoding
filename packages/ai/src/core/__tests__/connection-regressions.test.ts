...
const provider: Provider = {
  id: 'probe', userId: 'local', name: '测试', protocol: 'openai', baseUrl: 'http://127.0.0.1',
  headers: {}, keyRef: null, timeoutMs: 1000, supportsStream: true, supportsTools: false,
  supportsVision: false, enabled: true, order: 0, manualModels: ['local-model'],
  version: 1, createdAt: 0, updatedAt: 0,
};
const context = { apiKey: null, transport: { request: async (): Promise<never> => { throw new Error('不应调用'); } } };

describe('连接测试回归', () => {
  it('发现模型抛错而手填模型对话成功时返回成功', async () => {
    const adapter: ProviderAdapter = {
      protocol: 'openai',
      listModels: async () => { throw new ProtocolError('无模型列表'); },
  ...
  it('主动取消或不完整响应不能被报告为连通成功', async () => {
    const adapter: ProviderAdapter = {
      ...
      countTokens: () => ({ tokens: 0, estimated: true, margin: 0 }),
      async *chat() { yield { type: 'done', finishReason: 'aborted', partial: true }; },
    };
  ...
  it('错误消息、片段和序列化结果不暴露常见Key，cause不保留', () => {
    ...
    const error = new ProtocolError(`服务异常 ${key}`, { snippet: `api_key=${key}`, cause: new Error(key) });
